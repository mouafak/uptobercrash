...
4. **Déploiement Coolify**, dans cet ordre impératif :
   ...
   - **laisser vides** les champs *Build command*, *Start command* et les deux de *Deployment lifecycle*. Railpack exécute les scripts `build` et `start` de `package.json`, et la procédure de déploiement reste ainsi dans le dépôt ;
   - **les migrations ne peuvent pas tourner pendant le build** : le conteneur de build n'est pas attaché au réseau Docker du projet, le nom d'hôte interne de la base n'y résout pas, et `prisma migrate deploy` échoue en `P1001: Can't reach database server`. Aucun réglage ne l'y rattache — l'option *Connect To Predefined Network* ne concerne que le conteneur d'exécution. Elles tournent donc **au démarrage** : `"start": "prisma migrate deploy && next start"`. C'est aussi le meilleur moment, puisqu'un échec arrête le conteneur avant qu'il ne serve la moindre requête. *Pre-deployment* serait pire encore : il s'exécute dans le conteneur **existant**, dont le dossier `prisma/migrations` ne contient pas encore la migration qu'on apporte, et qui n'existe pas au premier déploiement ; *Post-deployment*, lui, s'exécute après la bascule du trafic ;
